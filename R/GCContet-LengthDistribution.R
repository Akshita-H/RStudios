...
n_reads <- 5000
# Simulating and Summarizing Sequence Length Distributions ----
seq_data <- tibble(
  Illumina = rnorm(n_reads, mean = 150, sd = 2),
  Nanopore = rlnorm(n_reads, meanlog = 8.5, sdlog = 0.6) # Right-skewed
) %>%
  pivot_longer(cols = everything(), names_to = "Platform", values_to = "Length")

# Calculate summary metrics (OpenIntro Ch. 2)
summary_stats <- seq_data %>%
  group_by(Platform) %>%
  summarise(
    Mean = mean(Length),
...
# Sliding-Window GC Content along a Sequence ----
# Generate a synthetic 50kb chromosome with a high-GC island
